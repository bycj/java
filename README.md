...
* xinan-demo-service，包含各中间件的使用示例代码，代码在src/main/java目录下的xinan.demo包中。
* xinan-demo-start，包含启动类`xinan.demo.Application`。中间件使用示例的单元测试代码在`src/test/java`目录下的`xinan.demo`包中。日志配置文件为`src/main/resources`目录下的logback-spring.xml。使用springmvc的代码在`src/main/java`目录下的`xinan.demo`包中，velocity模板在`/src/main/resources/templates`目录中。

## 使用方式
### 在开发工具中执行
将工程导入eclipse或者idea后，直接执行包含main方法的类`xinan.demo.Application`。
...
然后进入`xinan-demo-start/target`目录，执行fat jar

```sh
java -Dpandora.location=${sar} -jar xinan-demo-start-1.0.0-SNAPSHOT.jar
```
...
```sh
mvn -pl xinan-demo-start pandora-boot:run
```
